...
# ИСПРАВЛЕННЫЙ ИМПОРТ: Импортируем только genai, а константы используем через полный путь
from google.genai.types import SafetySetting, HarmBlockThreshold, HarmCategory as HCategory 
# Мы переименовали HarmCategory в HCategory, чтобы избежать любых конфликтов
# и будем использовать HarmCategory.HARM_CATEGORY_X через полный путь

# ==================================
# 1. Инициализация и настройки (Безопасное чтение ключей)
# ==================================

# Загружаем переменные из .env файла для локального запуска
load_dotenv() 

# Чтение ключей из окружения (эти ключи будут подставлены Render!)
BOT_TOKEN = os.getenv("BOT_TOKEN") 
GEMINI_API_KEY = os.getenv("GEMINI_API_KEY")

if not BOT_TOKEN or not GEMINI_API_KEY:
    # Ошибка только при локальном запуске, если .env не найден
    print("❌ ВНИМАНИЕ: Ключи BOT_TOKEN или GEMINI_API_KEY не найдены в локальном окружении.")
...
SAFETY_SETTINGS = [
    ...
    for category in [
        # ИСПОЛЬЗУЕМ ПОЛНЫЙ ПУТЬ К КОНСТАНТАМ ЧЕРЕЗ HarmCategory
