import os
import asyncio
from dotenv import load_dotenv # Добавлено для чтения .env
...
if not BOT_TOKEN or not GEMINI_API_KEY:
    print("❌ ОШИБКА: Ключи BOT_TOKEN или GEMINI_API_KEY не найдены!")
    print("Убедитесь, что файлы .env и .py находятся в одной папке.")
    exit()
...
if __name__ == "__main__":
    try:
        # Установка, что бот читает ключи из .env для локального запуска
        if not BOT_TOKEN or not GEMINI_API_KEY:
            print("\n🚨 ПОДСКАЗКА: Для локального запуска установите 'pip install python-dotenv'.")
